    ...
    *,
    ...
    if current is not None and current != raw and held_sec < min_hold_sec:
        ...
        hard_exit = raw == DeskMode.CERTAINTY and (
            ...
            or inputs.confirmed_pick_count < min_confirmed_picks
        )
        if not hard_exit:
            mode = current
...
def _overlays_for(
    ...
    *,
    ...
    target = 0.0
    if sat > 0:
        target = sat * float(velocity_ring_fraction_of_satellite)
    if cert > 0:
        target = max(target, cert * float(velocity_ring_mult_of_certainty))
