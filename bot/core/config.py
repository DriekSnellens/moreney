...
class Settings(BaseSettings):
    ...
    momentum_desk_fade_eta_sec: float = Field(default=0.0, ge=0.0, le=3600.0)
    ...
    momentum_short_weakest_book_eur: float = Field(default=5_000.0, gt=0)
    ...
    momentum_short_weakest_day_loss_limit_eur: float = Field(default=150.0, gt=0)
    momentum_short_weakest_week_loss_limit_eur: float = Field(default=400.0, gt=0)
    ...
    # When a base is on 2+ venues: primary keeps adds; secondary is sell-only + BE wind-down.
    live_micro_consolidate_duplicate_bases: bool = True
    live_micro_consolidate_primary_venue: str = "bitvavo"
    ...
    live_micro_low_util_buy_resting_max_age_sec: float = Field(
