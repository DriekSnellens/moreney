...
PARAMETER_CHANGES: list[dict[str, str]] = [
    ...
    {
        "file": "bot/opportunity/ev_engine.py",
        "change": "Maker EV = P(fill)×E(NET|fill) for trade-through-only fills",
        "old": "EV = p_fill × unconditional NET (independence assumption)",
        "new": "Charge extra conditional adverse beyond buffer when queue_fill=0",
        "reason": "€25k: fees match but gross evaporates; 5s markout mean ~21 bps vs ~5 bps buffer",
        "expected_effect": "Toxic trade-through quotes get negative EV before calibration",
        "risk": "If queue fills are later enabled, conditioning must be revisited",
    },
    {
        "file": "bot/opportunity/calibration.py",
        "change": "Early route stop on raw capture (n≥8, capture≤−0.25, loss≥€5)",
        "old": "Hard gate only when shrunk capture ≤ 0 (needs ~20 samples at prior_k=40)",
        "new": "Separate early stop using sum(realized)/sum(expected) without shrinkage",
        "reason": "bitvavo→bitvavo: early stop at n=5–8 would have avoided ~€40 of −€52",
        "expected_effect": "Stops structurally toxic routes before min_samples=20",
        "risk": "Rare false stop on a route that mean-reverts after 8 unlucky fills",
    },
    {
        "file": "bot/core/config.py + env/paper-*live.env",
        "change": "Markout adverse ceiling 15 → 40 bps",
        "old": "Ceiling 15 clipped suggested adverse below observed mean ~21",
        "new": "Ceiling 40 so gate can track measured toxicity",
        "reason": "Observed binding ceiling on Bitvavo 5s markouts",
        "expected_effect": "Higher required NET edge on toxic venues (more conservative)",
        "risk": "Fewer quotes pass profitability until markout improves",
    },
    {
        "file": "bot/paper/tracker.py",
        "change": "Realized adverse from waterfall identity, not EV gap",
        "old": "realized_adverse = expected_net − realized_net",
        "new": "adverse = gross − fees − slippage − realized (buffer excluded)",
        "reason": "EV gap mixed model error with adverse selection",
